...
export const botMessage = ({ context, message }) => async (
  dispatch,
  getState
) => {
  ...
  try {
    const res = await axios.post("/chat", body, tokenConfig(getState));
    ...
    dispatch(
      returnErrors(err.response.data, err.response.status, "ADD_RESPONSE_FAIL")
    );
    dispatch({
